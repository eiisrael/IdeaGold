...
class XMRigController extends EventEmitter {
  ...
  async start({wallet,workerName,pool,profile,reason='user'}){
    ...
    this.log('info','start','Iniciando XMRig.',{reason,profile:built.profile,args:args.map(a=>String(a).startsWith('4')?'[wallet/config-redacted]':a),exe:meta.exe});
    this.child=spawn(meta.exe,args,{cwd:path.dirname(meta.exe),stdio:['ignore',out,out],windowsHide:true,detached:false});
    const state={pid:this.child.pid,desired:'running',startedAt:Date.now(),profile:built.profile,configFile,configHash:this.configManager.hash(built.config),restartTimes:Array.isArray(current.restartTimes)?current.restartTimes:[],reason,lastExitAt:current.lastExitAt||null,lastExitCode:current.exitCode??null,lastExitSignal:current.exitSignal??null};this.writeJson(this.stateFile,state);this.currentProfile=built.profile;
