...
async function validateRuntime(){
  ...
  const set=await fetch(`${base}/api/settings`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({wallet,electricityBrlKWh:.9}),signal:AbortSignal.timeout(2000)});
  if(!set.ok)throw new Error(`Settings HTTP ${set.status}: ${await set.text()}`);
  const status=await fetch(`${base}/api/status`,{signal:AbortSignal.timeout(1500)}).then(r=>r.json());
  if(status?.settings?.wallet!==wallet)throw new Error('Status local não preservou a carteira pública completa para a UI.');
  ...
  const heartbeat=await fetch(`${base}/api/workers/heartbeat`,{method:'POST',headers:{'content-type':'application/json','x-ideagold-timestamp':timestamp,'x-ideagold-signature':signature},body:raw,signal:AbortSignal.timeout(2000)});
  if(!heartbeat.ok)throw new Error(`Worker heartbeat HTTP ${heartbeat.status}: ${await heartbeat.text()}`);
  const registry=await fetch(`${base}/api/workers`,{signal:AbortSignal.timeout(1500)}).then(r=>r.json());
  ...
  console.log(`OK smoke backend: ${body.name} ${body.version} + settings/wallet local + worker HMAC/SQLite`);
}
