...
function defaults(hardware){
  ...
  return {
    ...
    profile:normalizeConfig({threads,priority:3,yield:true,hugePages:true,hugePagesJit:true,rdmsr:true,wrmsr:true,numa:true,randomxMode:'fast',scratchpadPrefetch:1,pauseOnBattery:false,gpuMode:'off'}),
...
class SettingsStore{
  ...
  update(input={}){
    ...
    next.basePowerWatts=Math.max(0,Math.min(1000,Number(next.basePowerWatts??30)));
    next.cpuPowerWatts=Math.max(0,Math.min(1000,Number(next.cpuPowerWatts??75)));
