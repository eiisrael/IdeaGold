...
test('custo energia 100W 24h',()=>{const e=P.energyCost(100,1,86400);assert(Math.abs(e.kwh-2.4)<1e-9);assert(Math.abs(e.brl-2.4)<1e-9);});
...
test('quantum dinâmico não é valor fixo',()=>{assert.equal(niceQuantum(0.0000017),0.000002);assert.equal(niceQuantum(0.000007),0.00001);});
test('taxa observada usa crescimento do pool',()=>{const t=Date.now();const r=slopeRate([{ts:t,totalPoolXmr:1},{ts:t+1000,totalPoolXmr:1.001},{ts:t+2000,totalPoolXmr:1.002}]);assert(Math.abs(r.rate-.001)<1e-8);});
test('SQLite persiste settings/profiles/decisions',()=>{const dir=fs.mkdtempSync(path.join(os.tmpdir(),'ideagold-test-'));const db=new IdeaGoldDB(dir);db.setSetting('x',{a:1});assert.equal(db.getSetting('x').a,1);db.saveProfile({id:'p',name:'P',objective:'balanced',config:{threads:3}});assert.equal(db.listProfiles().length,1);db.addDecision({action:'TEST',reason:'unit'});assert.equal(db.listDecisions().length,1);db.db.close();fs.rmSync(dir,{recursive:true,force:true});});
