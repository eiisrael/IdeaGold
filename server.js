...
function requireLocal(req) {
  if (!isLoopback(req)) throw Object.assign(new Error('Controle do minerador disponível somente neste computador.'), { status: 403 });
}
...
function saveUser(body) {
  ...
  const wallet = String(body.wallet ?? old.wallet ?? '').trim();
  ...
  const data = {
    ...old,
    wallet,
    electricity: Math.max(0, Number(body.electricity ?? old.electricity ?? 0.90)),
    threads: [2, 3, 4].includes(Number(body.threads)) ? Number(body.threads) : (old.threads || 3),
    updatedAt: new Date().toISOString()
  };
  if (!data.createdAt) data.createdAt = data.updatedAt;
  writeJson(USER_FILE, data);
  audit('wallet_saved', { wallet: maskWallet(wallet), threads: data.threads });
  return data;
}
...
function findFile(dir, name) {
  ...
  for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (ent.isFile() && ent.name.toLowerCase() === name.toLowerCase()) return p;
    if (ent.isDirectory()) { const f = findFile(p, name); if (f) return f; }
  }
  return null;
}
...
async function installXmrig() {
  ...
  if (sha !== XMRIG_SHA256) {
    ...
    throw new Error('Falha de segurança: SHA-256 do XMRig não confere. Nada foi executado.');
  }

  const ps = spawnSync('powershell.exe', [
    '-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command',
    `Expand-Archive -LiteralPath '${zip.replaceAll("'", "''")}' -DestinationPath '${extract.replaceAll("'", "''")}' -Force`
  ...
  writeJson(MINER_META, meta);
  fs.rmSync(zip, { force: true });
  audit('xmrig_installed', { version: XMRIG_VERSION, sha256: sha });
  return meta;
}
...
async function startMiner() {
  ...
  if (!validXmrAddress(user.wallet)) throw Object.assign(new Error('Adicione sua carteira XMR antes de minerar.'), { status: 400 });
  let meta = minerMeta();
  if (!meta.installed || !meta.exe || !fs.existsSync(meta.exe)) meta = await installXmrig();

  const prev = readJson(PROCESS_FILE, {});
  if (pidAlive(prev.pid)) return { started: false, alreadyRunning: true, pid: prev.pid };

  writeJson(XMRIG_CONFIG, buildXmrigConfig(user));
  const out = fs.openSync(XMRIG_LOG, 'a');
  const err = fs.openSync(XMRIG_LOG, 'a');
  minerChild = spawn(meta.exe, ['-c', XMRIG_CONFIG, '--threads', String(user.threads)], {
    cwd: path.dirname(meta.exe), stdio: ['ignore', out, err], windowsHide: true, detached: false
  });
  writeJson(PROCESS_FILE, { pid: minerChild.pid, startedAt: new Date().toISOString(), wallet: maskWallet(user.wallet) });
  minerChild.on('exit', code => {
    audit('miner_exit', { code });
    const p = readJson(PROCESS_FILE, {});
    p.stoppedAt = new Date().toISOString(); p.exitCode = code;
    writeJson(PROCESS_FILE, p);
    minerChild = null;
  });
...
function stopMiner() {
  ...
  writeJson(PROCESS_FILE, p);
  minerChild = null;
  audit('miner_stop', { pid: p.pid || null });
  return { stopped: true };
}
...
async function poolStats(wallet) {
  ...
  for (const url of candidates) {
    try {
      const r = await fetch(url, { headers: { accept: 'application/json', 'user-agent': 'IdeaGold/3.0' }, signal: AbortSignal.timeout(5000) });
...
async function market() {
  ...
  marketCache.at = Date.now(); marketCache.data = d;
  return d;
}
...
function mapPool(d) {
  ...
  return {
    hashrate: pick('hash', 'hash2', 'hashrate'), totalHashes: pick('totalHash', 'totalHashes'),
    validShares: pick('validShares', 'valid'), invalidShares: pick('invalidShares', 'invalid'),
    dueAtomic: pick('amtDue', 'amountDue', 'balance'), paidAtomic: pick('amtPaid', 'amountPaid', 'paid'), raw: d
  };
}
function monteCarlo({ hashrate, price, electricity, difficulty, reward = 0.6, powerWatts = 115, days = 30 }) {
  ...
  out.sort((a, b) => a - b);
  const q = x => out[Math.min(out.length - 1, Math.floor(x * out.length))];
...
async function simpleStatus() {
  const user = userConfig();
  const meta = minerMeta();
  const proc = readJson(PROCESS_FILE, {});
  const sum = await xmrigSummary();
  const running = Boolean(sum) || pidAlive(proc.pid);
  const h = sum?.hashrate?.total || [];
  const conn = sum?.connection || {};
  ...
  return {
    version: '3.0.1', configured: validXmrAddress(user.wallet), wallet: maskWallet(user.wallet), walletFull: user.wallet || '', electricity: user.electricity || .90, threads: user.threads || 3,
    miner: {
      installed: Boolean(meta.installed && meta.exe && fs.existsSync(meta.exe)), version: meta.version || null, verified: Boolean(meta.verified), running,
      pid: proc.pid || null, hashrate, hashrate60s: Number(h[1] || 0), hashrate15m: Number(h[2] || 0),
      accepted: Number(conn.accepted || ps?.validShares || 0), rejected: Number(conn.rejected || ps?.invalidShares || 0), uptime: Number(sum?.uptime || 0), pool: conn.pool || `${POOL_HOST}:${POOL_PORT}`
    },
    pool: ps ? { ...ps, dueXmr: Number(ps.dueAtomic || 0) / 1e12, paidXmr: Number(ps.paidAtomic || 0) / 1e12 } : null,
    network: net,
    market: { xmrBRL, xmrUSD: Number(mkt?.monero?.usd || 0), change24h: Number(mkt?.monero?.brl_24h_change || 0) },
    risk
  };
}
function serveStatic(res, urlPath) {
  const file = (urlPath === '/' || urlPath === '/index.html') ? 'index.html' : null;
  if (!file) return false;
  const p = path.join(ROOT, file);
  if (!fs.existsSync(p)) { sendJson(res, 404, { error: 'arquivo não encontrado' }); return true; }
  const data = fs.readFileSync(p);
  res.writeHead(200, headers({ 'content-type': 'text/html; charset=utf-8', 'content-length': data.length, 'cache-control': 'no-cache' }));
  res.end(data);
  return true;
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && u.pathname === '/api/health') {
      return sendJson(res, 200, { ok: true, name: 'IdeaGold', version: '3.0.1', time: new Date().toISOString() });
    }
    if (req.method === 'GET' && u.pathname === '/api/simple/status') {
      return sendJson(res, 200, await simpleStatus());
    }
    if (req.method === 'POST' && u.pathname === '/api/simple/wallet') {
      requireLocal(req);
      return sendJson(res, 200, { ok: true, user: saveUser(await readBody(req)) });
    }
    if (req.method === 'POST' && u.pathname === '/api/simple/install') {
      requireLocal(req);
      return sendJson(res, 200, { ok: true, miner: await installXmrig() });
    }
    if (req.method === 'POST' && u.pathname === '/api/simple/start') {
      requireLocal(req);
      return sendJson(res, 200, { ok: true, result: await startMiner() });
    }
    if (req.method === 'POST' && u.pathname === '/api/simple/stop') {
      requireLocal(req);
      return sendJson(res, 200, { ok: true, result: stopMiner() });
    }
    if (req.method === 'POST' && u.pathname === '/api/simple/settings') {
      requireLocal(req);
      const b = await readBody(req);
      const cur = userConfig();
      return sendJson(res, 200, { ok: true, user: saveUser({ ...cur, ...b, wallet: cur.wallet }) });
    }
    return sendJson(res, 404, { error: 'Rota não encontrada' });
  } catch (e) {
    console.error(e);
    return sendJson(res, e.status || 500, { error: e.message || 'Erro interno' });
  }
});

server.listen(PORT, HOST, () => {
  console.log(`\nIdeaGold 3.0.1: http://${HOST}:${PORT}`);
  console.log('Controle local protegido: minerador só pode ser iniciado pelo próprio PC.');
});